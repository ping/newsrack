...
class NYTimesBooks(BasicNewsRecipe):
    ...
    def preprocess_raw_html(self, raw_html, url):
        ...
        for k, v in content_service["ROOT_QUERY"].items():
            if not k.startswith("workOrLocation"):
                continue
        ...
        document_block = content_service[
            article["sprinkledBody"]["id"]
        ]  # typename = "DocumentBlock"
        ...
        for c in document_block.get("content@filterEmpty", []):
            ...
            if c["typename"] in [
                ...
                if header_block.get("byline"):
                    ...
                    pub_dt_ele.string = ", ".join(
                        [
                            content_service[b["id"]]["renderedRepresentation"]
                            for b in content_service[header_block["byline"]["id"]][
                                "bylines"
                            ]
                        ]
                    )
